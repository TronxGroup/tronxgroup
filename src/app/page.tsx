...
import { ArrowRight, Building2, Film, Landmark, Rocket, Users, Link2 } from 'lucide-react';
    ...
    <>
      ...
      <section id="marcas" className="section border-t border-white/10 bg-gradient-to-b from-white/5 to-transparent">
        <div className="container">
          ...
          <p className="mt-2 text-white/70 max-w-2xl">Tronx Group opera con tres marcas propias. Además, se apalanca en alianzas estratégicas para ampliar capacidades y alcance.</p>
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-8">
            ...
            <BrandCard title="Tronx TV" description="Canal y formatos de contenido original (en desarrollo)." href="#" icon={<Rocket />} />
          </div>

          {/* Alianzas y relaciones */}
          <div className="mt-12 grid md:grid-cols-2 gap-6">
            <div className="card">
              <h3 className="text-xl font-semibold flex items-center gap-2"><Link2 size={18}/> Alianzas de Tronx Strategy</h3>
              <p className="text-white/70 mt-2">Tronx Strategy colabora con organizaciones y talentos para potenciar propuestas B2B.</p>
              <ul className="mt-4 space-y-2 text-white/80">
                <li>
                  <strong>APCC — Cámara de Comercio Asia Pacífico</strong>
                  <span className="block text-white/60 text-sm">Aliada de <em>Tronx Strategy</em> en membresías empresariales, misiones y foros.</span>
                  <span className="block text-white/50 text-xs mt-1">APCC mantiene alianza con <strong>HKLABA</strong> (Hong Kong – Latin America Business Association).</span>
                  <div className="mt-2 flex gap-3 text-sm">
                    <Link className="link" href="https://apcc-chamber.vercel.app">APCC →</Link>
                    <Link className="link" href="https://hklaba.vercel.app">HKLABA →</Link>
                  </div>
                </li>
                <li className="mt-3">
                  <strong>Echevensko</strong>
                  <span className="block text-white/60 text-sm">Aliado de <em>Tronx Strategy</em> para charlas corporativas y formación (La Magia de la Imaginación).</span>
                  <div className="mt-2 text-sm"><Link className="link" href="https://empresas.echevensko.com">Echevensko →</Link></div>
                </li>
              </ul>
            </div>
            <div className="card">
...
function Pill({ children }: { children: React.ReactNode }) {
  return <span className="inline-flex items-center rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 text-xs">{children}</span>;
}
...
function EcosystemMap() {
  return (
    <div className="space-y-3">
      <EcosystemRow left={<><Pill>Tronx Group</Pill></>} right={<div className="flex gap-1 flex-wrap"><Pill>Dekaelo Media</Pill><Pill>Tronx Strategy</Pill><Pill>Tronx TV</Pill></div>} />
      <EcosystemRow left={<><Pill>Tronx Strategy</Pill></>} right={<div className="flex gap-1 flex-wrap"><Pill>APCC</Pill><Pill>Echevensko</Pill></div>} />
      <EcosystemRow left={<><Pill>APCC</Pill></>} right={<div className="flex gap-1 flex-wrap"><Pill>HKLABA</Pill></div>} />
      <p className="text-xs text-white/60">Relaciones: Tronx Strategy ↔ aliados (APCC, Echevensko). APCC ↔ HKLABA.</p>
    </div>
  );
}
